...
def extract_game_info(file_path):
    ...
    # 尝试从分类部分提取更多分类
    category_section_match = re.search(r'<div class="categories-section">(.*?)</div>', content, re.DOTALL)
    if category_section_match:
        category_tags = re.findall(r'<span class="category-tag"[^>]*>(.*?)</span>', category_section_match.group(1))
        if category_tags:
            categories = [tag.strip() for tag in category_tags if tag.strip()]
...
def update_game_page(file_path, template_path, iframe_urls, all_games, games_by_category):
    ...
    try:
        ...
        category_matches = re.finditer(category_pattern, new_content, re.DOTALL)
        ...
        for match in category_matches:
            ...
            for category in game_info['categories']:
                category_html = template_part.replace('{{category}}', category)
                categories_html += category_html
            
            # 替换此匹配的循环块
            new_content = new_content[:match_pos] + categories_html + new_content[match.end():]
...
def main():
    ...
    # 输出分类统计
    category_counts = {category: len(games) for category, games in games_by_category.items()}
    print("\n分类统计:")
    for category, count in sorted(category_counts.items(), key=lambda x: x[1], reverse=True):
        print(f"{category}: {count} 游戏")
