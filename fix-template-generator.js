...
const OUTPUT_DIR = path.join(__dirname, './output-fixed');
...
// 处理模板中的Jinja类循环
function processJinjaFor(template, pattern, items, itemCallback) {
  const forRegex = new RegExp(pattern, 'g');
  let match;
  let processedTemplate = template;
  
  while ((match = forRegex.exec(template)) !== null) {
    const [fullMatch, loopContent] = match;
    let replacement = '';
    
    if (items && items.length > 0) {
      replacement = items.map(item => {
        return itemCallback(item, loopContent);
      }).join('');
    }
...
function generateGamePage(game) {
  ...
  // 处理游戏类别的循环
  pageContent = processJinjaFor(
    pageContent,
    '{%\\s*for\\s+category\\s+in\\s+game\\.categories\\s*%}([\\s\\S]*?){%\\s*endfor\\s*%}',
    categories,
    (category, content) => {
  ...
  pageContent = pageContent.replace(
    /{%\s*if\s+not\s+loop\.last\s*%}([\s\S]*?){%\s*endif\s*%}/g, 
    ', '
  );
  ...
  pageContent = processJinjaFor(
    pageContent,
    '{%\\s*for\\s+rec_game\\s+in\\s+game\\.recommendedGames\\s*%}([\\s\\S]*?){%\\s*endfor\\s*%}',
    gameData.recommendedGames,
    (recGame, content) => {
      let result = content;
      result = result.replace(/\{\{rec_game\.title\}\}/g, recGame.title);
      result = result.replace(/\{\{rec_game\.description\}\}/g, recGame.description);
      result = result.replace(/\{\{rec_game\.slug\}\}/g, recGame.slug);
      result = result.replace(/\{\{rec_game\.image\}\}/g, recGame.image);
      return result;
    }
  );
